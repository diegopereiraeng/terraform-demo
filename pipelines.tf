resource "harness_yaml_config" "k8s_rolling_workflow" {
  app_id  = harness_application.harnesstf.id
  path    = "Setup/Applications/${harness_application.harnesstf.name}/Workflows/k8s-rolling-deployment.yaml"
  content = file("${path.module}/templates/k8s-rolling-workflow.yaml")
}

resource "harness_yaml_config" "k8s_canary_workflow" {
  app_id  = harness_application.harnesstf.id
  path    = "Setup/Applications/${harness_application.harnesstf.name}/Workflows/k8s-canary-deployment.yaml"
...
resource "harness_yaml_config" "k8s_pipeline" {
  app_id  = harness_application.harnesstf.id
  path    = "Setup/Applications/${harness_application.harnesstf.name}/Pipelines/k8s-deployment.yaml"
